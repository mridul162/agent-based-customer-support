...
class Escalation(Base):
    ...
    queue: Mapped[EscalationQueue] = mapped_column(
        Enum(EscalationQueue), nullable=False, default=EscalationQueue.GENERAL
    )
    status: Mapped[EscalationStatus] = mapped_column(
        Enum(EscalationStatus), nullable=False, default=EscalationStatus.OPEN
    )
    ...
    def __repr__(self) -> str:
        return (
            f"<Escalation {self.escalation_id} queue={self.queue} status={self.status}>"
        )
